...
typedef  unsigned char* (*BilateralFilter)(unsigned char* ImageIn, unsigned char* ImageOut, int width, int height, int value);
typedef  unsigned char* (*GaussianFilter)(unsigned char* ImageIn, unsigned char* ImageOut, int width, int height, int sizeWidth, int sizeHeight, int sigmaX, int sigmaY);
...
int main()
{
	...
	Mat i_depthFilter(424, 512, CV_8UC1);
	...
	while (true)
	{
		...
		// filter
		uchar* temp = new uchar[512 * 424];
		i_depthFilter.data = DepthFilter2(i_depth.data, temp, 512, 424, 3, 3, 0, 0);
		...
		if (waitKey(1) == VK_ESCAPE)
			break;
		imshow("depthFilter", i_depthFilter);
		if (waitKey(1) == VK_ESCAPE)
			break;
		/*imshow("ir", i_ir);
		if (waitKey(1) == VK_ESCAPE)
